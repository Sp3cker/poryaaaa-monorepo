...
use crate::{
    params::{internals::ParamPtr, Param},
...
pub trait GuiContext: Send + Sync + 'static {
    ...
    /// Ask the host to deactivate and reactivate the plugin. Returns `false` when the current
    /// plugin API does not support this callback.
...
impl<'a> ParamSetter<'a> {
    ...
    /// Ask the host to deactivate and reactivate the plugin.
    pub fn request_restart(&self) -> bool {
        self.raw_context.request_restart()
    }
