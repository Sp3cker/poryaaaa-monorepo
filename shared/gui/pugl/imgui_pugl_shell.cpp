...
{
    return (mods & (PUGL_MOD_SHIFT | PUGL_MOD_CTRL | PUGL_MOD_ALT | PUGL_MOD_SUPER)) == 0;
}
...
bool isPlainSpaceEvent(const PuglEvent* event)
{
    ...
    if (event->type == PUGL_KEY_PRESS || event->type == PUGL_KEY_RELEASE)
        return event->key.key == PUGL_KEY_SPACE && hasPlainModifiers(event->key.state);
...
void renderFrame(ImGuiPuglShell* shell)
{
...
#ifdef __APPLE__
    auto* metalContext = static_cast<PuglMetalContext*>(puglGetContext(shell->view));
    if (!metalContext || !metalContext->renderPassDescriptor || !metalContext->commandBuffer ||
    ...
    if (shell->callbacks.drawFrame)
        shell->callbacks.drawFrame(shell->callbacks.userData, shell->width, shell->height);

    ImGui::Render();
#ifdef __APPLE__
    ImGui_ImplMetal_RenderDrawData(ImGui::GetDrawData(), metalContext->commandBuffer, metalContext->renderEncoder);
#else
    glViewport(0, 0, static_cast<int>(shell->width), static_cast<int>(shell->height));
...
PuglStatus onPuglEvent(PuglView* view, const PuglEvent* event)
{
    ...
    case PUGL_TEXT:
        if (shell->embedded && isPlainSpaceEvent(event) && !ImGui::GetIO().WantTextInput)
...
ImGuiPuglShell* imgui_pugl_shell_create(const ImGuiPuglShellConfig& config, const ImGuiPuglShellCallbacks& callbacks)
{
...
bool imgui_pugl_shell_set_parent(ImGuiPuglShell* shell, std::uintptr_t nativeParent)
{
...
bool imgui_pugl_shell_show(ImGuiPuglShell* shell)
{
    ...
    puglShow(shell->view, shell->embedded ? PUGL_SHOW_PASSIVE : PUGL_SHOW_RAISE);
    return true;
}
...
bool imgui_pugl_shell_set_size(ImGuiPuglShell* shell, std::uint32_t width, std::uint32_t height)
{
    if (!shell || !shell->view)
        return false;
    ...
    if (scale < 1.0)
        scale = 1.0;
    puglSetSizeHint(
        shell->view, PUGL_CURRENT_SIZE, static_cast<PuglSpan>(width * scale), static_cast<PuglSpan>(height * scale));
    return true;
}

void imgui_pugl_shell_get_size(const ImGuiPuglShell* shell, std::uint32_t* width, std::uint32_t* height)
{
    if (!width || !height)
        return;
    if (!shell)
