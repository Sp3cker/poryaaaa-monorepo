...
static PuglStatus puglMacMetalEnter(PuglView* view, const PuglExposeEvent* expose)
{
    PuglMetalView* drawView = (PuglMetalView*)view->impl->drawView;
    if (!drawView)
    {
    ...
    if (expose)
    {
        ...
        ctx->renderPassDescriptor.colorAttachments[0].clearColor = MTLClearColorMake(0.12, 0.12, 0.12, 1.0);

        ctx->renderEncoder = [ctx->commandBuffer renderCommandEncoderWithDescriptor:ctx->renderPassDescriptor];
...
static PuglStatus puglMacMetalLeave(PuglView* view, const PuglExposeEvent* expose)
{
    PuglMetalView* drawView = (PuglMetalView*)view->impl->drawView;
    if (!drawView)
    {
