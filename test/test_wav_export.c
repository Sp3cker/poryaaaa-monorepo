...
static void play_test_sequence(M4AEngine *engine)
{
    ...
    for (int n = 0; n < numNotes && samplePos < TOTAL_SAMPLES; n++) {
        ...
        render_engine(engine, outL + samplePos, outR + samplePos, renderLen);
        samplePos += renderLen;
        ...
        if (gapLen > 0) {
            render_engine(engine, outL + samplePos, outR + samplePos, gapLen);
            samplePos += gapLen;
        }
    }
    ...
    if (samplePos < TOTAL_SAMPLES) {
        ...
        render_engine(engine, outL + samplePos, outR + samplePos, chordLen);
        samplePos += chordLen;
        ...
        if (tailLen > 0) {
            render_engine(engine, outL + samplePos, outR + samplePos, tailLen);
        }
    }
