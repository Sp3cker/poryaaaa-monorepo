...
#    define PLUGIN_LOG_RESTORE_FORMAT_NONLITERAL _Pragma("clang diagnostic pop")
#elif defined(__GNUC__)
#    define PLUGIN_LOG_DISABLE_FORMAT_NONLITERAL                                                                       \
        _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wformat-nonliteral\"")
#    define PLUGIN_LOG_RESTORE_FORMAT_NONLITERAL _Pragma("GCC diagnostic pop")
#else
#    define PLUGIN_LOG_DISABLE_FORMAT_NONLITERAL
    ...
    if (data->gui)
    {
        ...
        if (data->assetIndex)
            m4a_gui_set_project_assets(data->gui,
                                       data->assetIndex->directsoundAssets,
                                       data->assetIndex->directsoundCount,
                                       data->assetIndex->progWaveAssets,
                                       data->assetIndex->progWaveCount,
...
static void process_midi_clock_pulse(M4APluginData* data, uint32_t sample_in_block)
{
    ...
    if (instBpm < 20.0 || instBpm > 999.0)
        return; /* reject obvious jitter spikes */
...
static void process_midi_system_event(M4APluginData* data, const uint8_t* msg, uint32_t sample_in_block)
{
    switch (msg[0])
    {
...
static void process_midi_event(
    M4APluginData* data, const uint8_t* msg, uint32_t sample_in_block, bool has_recorder_beats, double recorder_beats)
{
    ...
    if (atomic_load(&data->recorderArmed) && has_recorder_beats)
    {
        ...
        if (status == 0xC0)
            atomic_fetch_or(&data->recorderSeenPC, bit);
        else if (status == 0xB0 && msg[1] == 0x07)
            atomic_fetch_or(&data->recorderSeenVol, bit);
        else if (status == 0xB0 && msg[1] == 0x0A)
            atomic_fetch_or(&data->recorderSeenPan, bit);
    }
}

static void process_clap_note_event(M4APluginData* data,
                                    const clap_event_note_t* ev,
                                    bool has_recorder_beats,
...
{
    return transport && (transport->flags & CLAP_TRANSPORT_HAS_TEMPO) && transport->tempo > 0.0;
}

static bool transport_has_beats_timeline(const clap_event_transport_t* transport)
{
    return transport && (transport->flags & CLAP_TRANSPORT_HAS_BEATS_TIMELINE);
}
...
static double recorder_beat_mapper_beats_at(const RecorderBeatMapper* mapper, uint32_t frame)
{
    if (!mapper->hasBeats || !mapper->hasTempo || mapper->sampleRate <= 0.0)
        return 0.0;

    double deltaFrames = frame > mapper->segmentFrame ? (double)(frame - mapper->segmentFrame) : 0.0;
    double beatDelta = (mapper->tempoBpm * deltaFrames + 0.5 * mapper->tempoInc * deltaFrames * deltaFrames) /
                                                 ...
                                                 uint32_t frame)
{
    ...
    mapper->hasBeats =
        mapper->hasTempo && mapper->sampleRate > 0.0 && (mapper->hasBeats || transport_has_beats_timeline(transport));
}
    ...
    while (framePos < numFrames)
    {
        ...
        while (eventIdx < numEvents)
        {
            ...
            if (hdr->space_id == CLAP_CORE_EVENT_SPACE_ID)
            {
                ...
                {
                    double recorder_beats = recorder_beat_mapper_beats_at(&recorderMapper, hdr->time);
                    process_clap_note_event(
                        data, (const clap_event_note_t*)hdr, recorderMapper.hasBeats, recorder_beats);
                    break;
                }
                case CLAP_EVENT_MIDI:
                {
                    ...
                    double recorder_beats = recorder_beat_mapper_beats_at(&recorderMapper, hdr->time);
                    process_midi_event(data, midiEv->data, hdr->time, recorderMapper.hasBeats, recorder_beats);
                ...
                case CLAP_EVENT_TRANSPORT:
                    recorder_beat_mapper_apply_transport(
                        data, &recorderMapper, (const clap_event_transport_t*)hdr, hdr->time);
                    break;
...
static void timer_on_timer(const clap_plugin_t* plugin, clap_id timer_id)
{
    ...
    if (reloadVoicegroup)
    {
        ...
        snprintf(data->projectRoot, sizeof(data->projectRoot), "%s", gs.projectRoot);
        snprintf(data->voicegroupName, sizeof(data->voicegroupName), "%s", gs.voicegroupName);
    ...
    const clap_host_undo_t* hostUndo = (const clap_host_undo_t*)data->host->get_extension(data->host, CLAP_EXT_UNDO);
    if (hostUndo && hostUndo->change_made)
    {
        const char* name = reloadVoicegroup ? "M4A: Reload Voicegroup" : "M4A: Settings Change";
...
static void plugin_handle_extract_request(M4APluginData* data)
{
    ...
    if (!voicegroup_channel_export_default_path(
            data->projectRoot, data->voicegroupName, outputPath, sizeof(outputPath)))
    {
        m4a_gui_set_extract_status(data->gui, "Failed: bad output path");
        return;
    }

    bool ok = voicegroup_export_channel_remap(
        data->projectRoot, data->voicegroupName, &data->loaderConfig, programs, outputPath);
    char status[256];
...
static bool gui_is_api_supported(const clap_plugin_t* plugin, const char* api, bool is_floating)
{
    ...
    plugin_log("gui_is_api_supported: api=%s floating=%d -> %d", api ? api : "(null)", is_floating, supported);
    return supported;
}
...
factory_create_plugin(const clap_plugin_factory_t* factory, const clap_host_t* host, const char* plugin_id)
{
    (void)factory;
