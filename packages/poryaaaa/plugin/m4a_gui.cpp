...
#    define GUI_LOG_RESTORE_FORMAT_NONLITERAL _Pragma("clang diagnostic pop")
#elif defined(__GNUC__)
#    define GUI_LOG_DISABLE_FORMAT_NONLITERAL                                                                          \
        _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wformat-nonliteral\"")
#    define GUI_LOG_RESTORE_FORMAT_NONLITERAL _Pragma("GCC diagnostic pop")
#else
#    define GUI_LOG_DISABLE_FORMAT_NONLITERAL
...
static bool project_root_button(const char* label, const ImVec2& size)
{
    ...
    ImVec2 textPos(min.x + ImGui::GetStyle().FramePadding.x, min.y + (max.y - min.y - textSize.y) * 0.5f);
...
struct M4AGuiState
{
    ...
    ImGui::FileBrowser projectRootBrowser{ImGuiFileBrowserFlags_SelectDirectory |
                                              ImGuiFileBrowserFlags_HideRegularFiles |
                                              ImGuiFileBrowserFlags_EditPathString | ImGuiFileBrowserFlags_CloseOnEsc |
...
{
    snprintf(gui->projectRootBuf, sizeof(gui->projectRootBuf), "%s", gui->settings.projectRoot);
    snprintf(gui->voicegroupBuf, sizeof(gui->voicegroupBuf), "%s", gui->settings.voicegroupName);
}
...
static bool edit_directsound_adsr(ToneData* voice)
{
    ...
    if (ImGui::SliderInt("Attack##ds", &a, 0, 255))
    {
        voice->attack = (uint8_t)a;
        changed = true;
    }
    if (ImGui::SliderInt("Decay##ds", &d, 0, 255))
    {
        voice->decay = (uint8_t)d;
        changed = true;
    }
    if (ImGui::SliderInt("Sustain##ds", &s, 0, 255))
    {
        voice->sustain = (uint8_t)s;
        changed = true;
    }
    if (ImGui::SliderInt("Release##ds", &r, 0, 255))
    {
        voice->release = (uint8_t)r;
        changed = true;
    }
    return changed;
}
...
static bool edit_cgb_adsr(ToneData* voice)
{
    bool changed = false;
    int a = voice->attack, d = voice->decay, s = voice->sustain, r = voice->release;
    if (ImGui::SliderInt("Attack##cgb", &a, 0, 7))
    {
        voice->attack = (uint8_t)a;
        changed = true;
    }
    if (ImGui::SliderInt("Decay##cgb", &d, 0, 7))
    {
        voice->decay = (uint8_t)d;
        changed = true;
    }
    if (ImGui::SliderInt("Sustain##cgb", &s, 0, 15))
    {
        voice->sustain = (uint8_t)s;
        changed = true;
    }
    if (ImGui::SliderInt("Release##cgb", &r, 0, 7))
    {
        voice->release = (uint8_t)r;
        changed = true;
    }
...
static void render_general_tab(M4AGuiState* gui)
{
    ...
    ImGui::TextUnformatted("MIDI Activity");
    {
        ...
        for (int ch = 0; ch < 16; ch++)
        {
            ...
            drawList->AddCircleFilled(c, 5.0f, ImGui::GetColorU32(active ? kChanOn : kChanOff));
        }
    ...
    {
        if (copy_path_utf8(gui->projectRootBuf, sizeof(gui->projectRootBuf), gui->projectRootBrowser.GetSelected()))
        {
    ...
    ImGui::SameLine();
    {
        ...
        ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x - reloadW - spacing);
    }
 ...
 */
static void render_sample_selector(M4AGuiState* gui,
                                   int voiceIndex,
                                   const char* label,
                                   ...
                                   std::span<const ProjectAssetEntry> assets)
{
    ...
    if (ImGui::BeginCombo("##sampleCombo", preview))
    {
        ...
        {
            if (gui->sampleFilterText[0] != '\0' && ci_strstr(asset.fileName, gui->sampleFilterText) == NULL)
                continue;
            ...
            if (ImGui::Selectable(asset.fileName, isSelected))
            {
                ...
                strncpy(gui->pendingSwapFileName, asset.fileName, sizeof(gui->pendingSwapFileName) - 1);
...
static void render_voices_tab(M4AGuiState* gui)
{
    ...
    if (gui->selectedVoice < 0)
        gui->selectedVoice = 0;
    if (gui->selectedVoice > 127)
        gui->selectedVoice = 127;
    ...
    if (baseType == 0x00)
    {
        ...
        render_sample_selector(gui, idx, "Sample", PROJECT_ASSET_DIRECTSOUND, gui->directsoundAssets);
    }
    else if (baseType == 0x01)
    {
        /* Square 1 */
        int key = voice->key;
        if (ImGui::SliderInt("Key", &key, 0, 127))
        {
            voice->key = (uint8_t)key;
            changed = true;
        }
        int sweep = voice->panSweep;
        if (ImGui::SliderInt("Sweep", &sweep, 0, 127))
        {
            voice->panSweep = (uint8_t)sweep;
            changed = true;
        }
    ...
    else if (baseType == 0x03)
    {
        ...
        render_sample_selector(gui, idx, "Wave", PROJECT_ASSET_PROG_WAVE, gui->progWaveAssets);
    }
...
static void render_recorder_tab(M4AGuiState* gui)
{
    ...
    if (ImGui::Button("Save SMF"))
    {
        ...
        {
            snprintf(gui->recorderStatus, sizeof(gui->recorderStatus), "Failed: host tempo required");
#if !defined(_WIN32)
        }
        else if (strchr(data->recorderPath, '\\'))
        {
            snprintf(gui->recorderStatus, sizeof(gui->recorderStatus), "Failed: use / path separators");
#endif
        }
        else
        {
            bool ok = m4a_recorder_save_smf(data->recorder, data->recorderPath, 96, data->recorderTempoBpm);
            snprintf(
                gui->recorderStatus, sizeof(gui->recorderStatus), ok ? "Saved: %s" : "Failed: %s", data->recorderPath);
        }
    }
    ...
    {
        bool on = (mask >> ch) & 1u;
        ImGui::TextColored(on ? ImVec4(0.18f, 0.95f, 0.35f, 1.0f) : ImVec4(0.18f, 0.24f, 0.20f, 1.0f), "\xE2\x97\x8F");
    };
...
static void shell_closed(void* userData, bool wasDestroyed)
{
    ...
    if (gui->host)
    {
        const clap_host_gui_t* hostGui = (const clap_host_gui_t*)gui->host->get_extension(gui->host, CLAP_EXT_GUI);
        if (hostGui)
...
extern "C"
{
    ...
    {
        return gui && poryaaaa::gui::imgui_pugl_shell_set_size(gui->shell, width, height);
    }
    ...
    void m4a_gui_set_voice_data(M4AGuiState* gui, ToneData* liveVoices, const ToneData* originalVoices, bool* overrides)
    {
        if (!gui)
            return;
        (void)originalVoices;
    ...
    bool m4a_gui_poll_sample_swap(
        M4AGuiState* gui, int* voiceIndex, ProjectAssetKind* kind, char* fileName, int fileNameSize)
    {
