...
/* Legacy channel status names retained for source compatibility. */
...
{
    uint8_t numSteps;
    uint8_t duty[MAX_PWM_PATTERN_STEPS];
...
struct M4AEngine
{
    /* Legacy public telemetry and direct-write surface. */
    ...
    /* v2 extension surface retained for newer clients. */
    M4ADriver* driver;
    HwAudio* hw;
    uint8_t volume;
    ...
    /* Private v2 sidecars backing invert-only lost-sound and audition audio. */
...
/* Lifecycle.  bool is the v2 extension return type; legacy callers may ignore it. */
