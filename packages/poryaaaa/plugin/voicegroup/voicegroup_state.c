...
bool voicegroup_state_write_default(const char* projectRoot, const char* voicegroupName, const LoadedVoiceGroup* vg)
{
    if (!projectRoot || !voicegroupName || !vg)
        return false;

    char stateDir[600];
    if (!resolve_state_dir(stateDir, sizeof(stateDir)))
        return false;
    mkdir_p(stateDir);
