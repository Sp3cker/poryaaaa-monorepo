...
static int try_file_in_subdir(const char* dir, const char* subdir, const char* name, ChannelExportLocation* out)
{
    char path[VG_MAX_PATH_LEN];
...
static int try_suffix_convention(const char* voicegroupName,
                                 const char* suffix,
                                 const char* subdir,
...
static int
find_in_monolithic_files(const char* voicegroupName, const ProjectDiscovery* disc, ChannelExportLocation* out)
{
...
static ChannelExportLocation
find_voicegroup_source(const char* projectRoot, const char* voicegroupName, const VoicegroupLoaderConfig* config)
{
    ...
    if (try_suffix_convention(voicegroupName, "_keysplit", "keysplits", &disc, &loc))
        return loc;
    if (try_suffix_convention(voicegroupName, "_drumset", "drumsets", &disc, &loc))
        return loc;
...
{
    if (strstr(trimmed, ".align") == trimmed)
        return 1;
    if (strstr(trimmed, "::") && trimmed[0] != '.')
        return 1;
    return 0;
}
...
static bool read_voice_lines(const ChannelExportLocation* loc, char voiceLines[VOICEGROUP_SIZE][VG_MAX_LINE])
{
    ...
    while (fgets(line, sizeof(line), f) && voiceIndex < VOICEGROUP_SIZE)
    {
        ...
        if (strncmp(trimmed, "voice_group ", 12) == 0)
        {
            ...
            int startingNote = 0;
            if (sscanf(trimmed + 12, "%[^,\n], %d", ignored, &startingNote) >= 2 && startingNote > 0 &&
