...
static bool keysplit_map_reserve(KeySplitMap* map, int needed)
{
    ...
    KeySplitDef* entries = vg_realloc_array(map->entries, (size_t)newCapacity, sizeof(*map->entries));
    if (!entries)
        return false;
 ...
 */
static bool parse_emerald_line(const char* trimmed, KeySplitMap* map, KeySplitDef** current, int* lastNote)
{
 ...
 */
static bool parse_firered_line(const char* trimmed, KeySplitMap* map, KeySplitDef** current, int* lastNote)
{
