...
static int read_wav_chunks(FILE* f, const char* absoluteWavPath, WavChunkData* out)
{
    ...
    uint8_t riffHdr[12];
    if (fread(riffHdr, 1, 12, f) != 12 || memcmp(riffHdr, "RIFF", 4) != 0 || memcmp(riffHdr + 8, "WAVE", 4) != 0)
    {
    ...
    while (1)
    {
        ...
        if (chunkLen & 1)
            nextChunk++; /* chunks align to even bytes */
...
static uint32_t bytes_per_sample(const WavChunkData* c, const char* absoluteWavPath)
{
    if (c->fmtTag == 1)
    {
        if (c->blockAlign == 1 && c->bitsPerSample == 8)
            return 1;
        if (c->blockAlign == 2 && c->bitsPerSample == 16)
            return 2;
        if (c->blockAlign == 3 && c->bitsPerSample == 24)
            return 3;
        if (c->blockAlign == 4 && c->bitsPerSample == 32)
            return 4;
    ...
    if (c->fmtTag == 3)
    {
        if (c->blockAlign == 4 && c->bitsPerSample == 32)
            return 4;
        if (c->blockAlign == 8 && c->bitsPerSample == 64)
            return 8;
...
static uint32_t compute_freq(const WavChunkData* c)
{
    ...
    double pitch = (double)c->sampleRate * pow(2.0, (60.0 - (double)c->midiKey) / 12.0 + tuning / 1200.0);
