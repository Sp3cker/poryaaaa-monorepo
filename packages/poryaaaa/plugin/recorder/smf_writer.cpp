...
bool write_smf1(const std::string& path, const RecorderCore::Snapshot& snapshot, const SmfWriteOptions& options)
{
    ...
    try
    {
        ...
        auto coalesces = [](const MidiRecord& rec)
        {
            ...
            if (kind == 0xB0)
            {
                if ((rec.status & 0xF0) != 0xB0)
                    return false; // redundant but for clarity
            ...
            if (ev.coalesces)
            {
                for (PendingMidiEvent& existing : events)
                {
                    if (!existing.coalesces)
                        continue;
                    if (existing.tick == ev.tick && existing.coalesceKind == ev.coalesceKind &&
        ...
        for (const auto& ip : options.initialPrograms)
        {
            if (ip.channel >= 16 || ip.program > 127)
                continue;
            if (channelTrack[ip.channel] < 0)
                continue;
            MidiRecord rec{};
        ...
        for (const auto& ic : options.initialCcs)
        {
            if (ic.channel >= 16 || ic.cc > 127 || ic.value > 127)
                continue;
            if (channelTrack[ic.channel] < 0)
                continue;
            MidiRecord rec{};
        ...
        for (int channel = 0; channel < 16; ++channel)
        {
            ...
            for (const PendingMidiEvent& event : pendingEvents[channel])
            {
                ...
                const std::uint8_t kind = m.status & 0xF0;
                const std::uint16_t key = static_cast<std::uint16_t>((channel << 8) | m.data1);
