 ...
 */
static void parse_track(MidiReader* r,
                        uint32_t trackLen,
                        uint8_t trackIndex,
                        RawEventArray* rawEvents,
                        TempoArray* tempos,
                        uint64_t* loopStartTick,
                        uint64_t* loopEndTick)
{
    ...
    while (r->pos < end)
    {
        ...
        if (b == 0xFF)
        {
            ...
            uint8_t metaType;
            uint32_t metaLen;
            if (mr_read_byte(r, &metaType) < 0)
                break;
            if (mr_read_vlq(r, &metaLen) < 0)
                break;
            ...
            if (metaType == 0x51 && metaLen == 3)
            {
                /* Tempo change */
                uint8_t t0, t1, t2;
                if (mr_read_byte(r, &t0) < 0 || mr_read_byte(r, &t1) < 0 || mr_read_byte(r, &t2) < 0)
                    break;
                uint32_t tempo = ((uint32_t)t0 << 16) | ((uint32_t)t1 << 8) | t2;
            ...
            else if (metaType >= 0x01 && metaType <= 0x07)
            {
                ...
                {
                    if (mr_read_byte(r, &textBuf[j]) < 0)
                        goto track_done;
                }
                if (metaLen > readLen)
                {
                    if (mr_skip(r, metaLen - readLen) < 0)
                        goto track_done;
                }
        ...
        else if (b == 0xF0 || b == 0xF7)
        {
            ...
            runningStatus = 0;
            uint32_t sysexLen;
            if (mr_read_vlq(r, &sysexLen) < 0)
                break;
            if (mr_skip(r, sysexLen) < 0)
                break;
        }
        else
        {
            ...
            case 0xD:
            { /* Channel Pressure — skip (already read data0) */
                break;
            }
            case 0xE:
            { /* Pitch Bend (2 data bytes: LSB already in data0) */
                uint8_t msb;
                if (mr_read_byte(r, &msb) < 0)
                    goto track_done;
 ...
 */
static uint64_t
tick_to_sample(uint64_t tick, const TempoEvent* tempos, int tempoCount, uint32_t tpqn, double sampleRate)
{
    double samples = 0.0;
    uint64_t prevTick = 0;
 ...
 */
static RenderEventArray* parse_midi(const char* path,
                                    double sampleRate,
                                    ...
                                    uint16_t* midiFormatOut)
{
    ...
    *loopEndSampleOut = UINT64_MAX;
    ...
    if (mr_read_u32_be(&r, &hdrLen) < 0 || mr_read_u16_be(&r, &format) < 0 || mr_read_u16_be(&r, &numTracks) < 0 ||
    ...
    *midiFormatOut = format;
    ...
    for (int t = 0; t < (int)numTracks; t++)
    {
        ...
        r.pos += 4;
        uint32_t trackLen;
        if (mr_read_u32_be(&r, &trackLen) < 0)
            break;
    ...
    *totalMidiSamples = tick_to_sample(lastTick, tempos.events, tempos.count, tpqn, sampleRate);

    /* ---- Convert loop marker ticks to sample positions ---- */
    if (loopStartTick != UINT64_MAX)
        *loopStartSampleOut = tick_to_sample(loopStartTick, tempos.events, tempos.count, tpqn, sampleRate);
    if (loopEndTick != UINT64_MAX)
        *loopEndSampleOut = tick_to_sample(loopEndTick, tempos.events, tempos.count, tpqn, sampleRate);
    ...
    qsort(result->events, (size_t)result->count, sizeof(RenderEvent), cmp_render_events);
 ...
 */
static void alsa_error_noop(const char* file, int line, const char* func, int err, const char* fmt, ...)
{
    (void)file;
    (void)line;
    (void)func;
...
static void playback_callback(ma_device* dev, void* out, const void* in, ma_uint32 frameCount)
{
    (void)in;
...
static uint32_t parse_solo_name(const char* name)
{
    ...
    if (strcmp(name, "full") == 0)
        return HW_AUDIO_SOLO_FULL;
    if (strcmp(name, "psg") == 0)
        return HW_AUDIO_SOLO_PSG;
    if (strcmp(name, "directsound") == 0)
        return HW_AUDIO_SOLO_DSOUND;
    if (strcmp(name, "ch1") == 0 || strcmp(name, "sq1") == 0 || strcmp(name, "square1") == 0)
        return HW_AUDIO_SOLO_SQ1;
    if (strcmp(name, "ch2") == 0 || strcmp(name, "sq2") == 0 || strcmp(name, "square2") == 0)
        return HW_AUDIO_SOLO_SQ2;
    if (strcmp(name, "ch3") == 0 || strcmp(name, "wave") == 0)
        return HW_AUDIO_SOLO_WAVE;
    if (strcmp(name, "ch4") == 0 || strcmp(name, "noise") == 0)
        return HW_AUDIO_SOLO_NOISE;
    if (strcmp(name, "fifo-a") == 0 || strcmp(name, "dma-a") == 0 || strcmp(name, "dsa") == 0)
        return HW_AUDIO_SOLO_DMA_A;
    if (strcmp(name, "fifo-b") == 0 || strcmp(name, "dma-b") == 0 || strcmp(name, "dsb") == 0)
        return HW_AUDIO_SOLO_DMA_B;
    return 0;
}
#endif
...
int main(int argc, char* argv[])
{
    ...
    RenderEventArray* events =
        parse_midi(midiPath, sampleRate, &totalMidiSamples, &loopStartSample, &loopEndSample, &midiFormat);
    if (!events)
        return 1;
    ...
    bool hasLoop = (loopStartSample != UINT64_MAX && loopEndSample != UINT64_MAX && loopEndSample > loopStartSample);

    if (!hasLoop && (loopStartSample != UINT64_MAX || loopEndSample != UINT64_MAX))
        fprintf(stderr,
                "Warning: incomplete loop markers (need both '[' and ']' "
    ...
    if (hasLoop)
    {
        ...
        if (totalDurSeconds >= 0.0)
        {
            ...
            fadeStartSample = totalSamples > fadeoutSamps ? totalSamples - fadeoutSamps : 0;
        }
        else
        {
        ...
        if (loopDuration > 0)
        {
            for (uint64_t off = 0; loopStartSample + off < totalSamples; off += loopDuration)
            {
                for (int i = 0; i < events->count; i++)
                {
                    ...
                    if (op < loopStartSample || op > loopEndSample)
                        continue;
                    uint64_t sp = op + off;
                    if (sp >= totalSamples)
                        continue;
    ...
    if (soloName)
    {
        fprintf(stderr, "Warning: --solo requires HW_AUDIO_V2 builds; ignoring '%s'.\n", soloName);
    }
#endif
    ...
    for (int i = 0; i < renderEvtCount; i++)
    {
        ...
        if (ev->samplePos >= totalSamples)
            break; /* safety: don't write past buffer */
