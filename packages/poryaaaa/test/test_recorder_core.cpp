...
{
    return ((uint32_t)bytes[offset] << 24) | ((uint32_t)bytes[offset + 1] << 16) | ((uint32_t)bytes[offset + 2] << 8) |
...
bool read_vlq(const std::vector<unsigned char>& bytes, size_t end, size_t& pos, int& value)
{
    value = 0;
    for (int i = 0; i < 4; i++)
    {
...
std::vector<ParsedMidiEvent> parse_channel_events(const char* path)
{
    ...
    {
        if (pos + 8 > bytes.size() || bytes[pos] != 'M' || bytes[pos + 1] != 'T' || bytes[pos + 2] != 'r' ||
            bytes[pos + 3] != 'k')
            return events;
...
bool has_event(const std::vector<ParsedMidiEvent>& events, int tick, std::initializer_list<unsigned char> bytes)
{
                                   ...
                                   unsigned char data1)
{
    ...
    for (const ParsedMidiEvent& event : events)
    {
        if (event.bytes.size() >= 2 && event.bytes[0] == status && event.bytes[1] == data1)
            count++;
    }
    return count;
}
...
void test_recorder_bridge_rejects_posix_backslashes()
{
    ...
    bool ok = m4a_recorder_save_smf(recorder, "\\Users\\spencer\\testf1.mid", 96, 120.0);
...
void test_smf_writer_coarse_grid_and_latest_value()
{
    ...
    ASSERT(count_events_with_status_data1(events, 0xB0, 0x1E) == 1, "XCMD selector is not duplicated");
    std::remove(path);
}
...
void test_smf_writer_normalizes_note_ticks()
{
    ...
    std::vector<ParsedMidiEvent> events = parse_channel_events(path);
    ASSERT(has_event(events, 0, {0x90, 60, 100}), "first note rounds to export start");
    ASSERT(has_event(events, 4, {0xB0, 7, 70}), "controller event still moves backward to coarse grid");
    ASSERT(has_event(events, 24, {0x90, 62, 100}), "note on rounds to nearest PPQ tick");
    ASSERT(has_event(events, 48, {0x80, 60, 0}), "note off rounds to nearest PPQ tick");
    ASSERT(has_event(events, 72, {0x80, 62, 0}), "second note off rounds to nearest PPQ tick");
    std::remove(path);
}
