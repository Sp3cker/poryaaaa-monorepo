...
void append_generated(std::uint32_t time, const std::vector<MidiEvent>& events, std::vector<TimedMidiEvent>* out)
{
...
{
    const auto rounded = static_cast<std::uint32_t>(std::max(0.0, std::round(position)));
...
void advance_to(Plugin* plugin, std::uint32_t fromTime, std::uint32_t toTime, std::vector<TimedMidiEvent>* out)
{
    ...
    while (plugin->core.has_active_ramp() && remaining + 0.000001 >= plugin->samplesUntilNextStep)
    {
    ...
    if (plugin->core.has_active_ramp())
    {
        plugin->samplesUntilNextStep = std::max(0.000001, plugin->samplesUntilNextStep - remaining);
    }
    else
    {
        plugin->samplesUntilNextStep = stepSamples;
...
bool event_to_midi(const clap_event_header_t* header, MidiEvent* midi)
{
    ...
    const auto channel = static_cast<std::uint8_t>(std::clamp(static_cast<int>(event->channel), 0, 15));
    const auto key = static_cast<std::uint8_t>(std::clamp(static_cast<int>(event->key), 0, 127));
    auto velocity =
        static_cast<std::uint8_t>(std::clamp(static_cast<int>(std::lround(event->velocity * 127.0)), 0, 127));
...
void push_midi_event(const TimedMidiEvent& event, const clap_output_events_t* outEvents)
{
    if (!outEvents)
        return;
...
clap_process_status plugin_process(const clap_plugin_t* plugin, const clap_process_t* process)
{
    ...
    output.reserve(eventCount + 16);
    {
        ...
        for (std::uint32_t i = 0; i < eventCount; ++i)
        {
            ...
            if (!header)
                continue;
            const std::uint32_t eventTime = std::min(header->time, process->frames_count);
...
bool params_value_to_text(const clap_plugin_t* plugin, clap_id paramId, double value, char* display, std::uint32_t size)
{
    ...
    if (paramId == kParamBendCurve)
    {
        std::snprintf(display, size, "%s", curve_from_value(value) == BendCurve::Easing ? "Easing" : "Linear");
        return true;
    }
    return false;
}
