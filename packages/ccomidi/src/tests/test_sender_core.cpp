...
void test_defaults_initialize_volume_and_pan()
{
    ...
    ASSERT_EQ(static_cast<int>(core.row_value_raw(kVolumeRow, 0)), 64, "default volume row value is 64");
    ASSERT_EQ(static_cast<int>(core.row_value_raw(kPanRow, 0)), 64, "default pan row value is 64");

    core.process_block(TransportState{true}, nullptr, 0, &planned);

    ASSERT_EQ(planned.count, 3, "default snapshot emits program change, volume, and pan");
...
void test_output_channel_is_limited_to_twelve_channels()
{
    ...
    ASSERT_EQ(core.output_channel(), ccomidi::kMaxSelectableOutputChannelIndex, "output channel floors to channel 12");
    ASSERT_EQ(core.output_channel_value(), 11.0, "stored output channel is floored to channel 12");

    core.set_output_channel(12.0);
    ASSERT_EQ(
        core.output_channel(), ccomidi::kMaxSelectableOutputChannelIndex, "output channel clamps above channel 12");
    ASSERT_EQ(core.output_channel_value(), 11.0, "stored output channel is clamped to channel 12");

    const AutomationEvent event = {0, ParamAddress{ParamKind::OutputChannel, 0}, 15.0};
    core.apply_parameter_change(event, nullptr, nullptr);
    ASSERT_EQ(core.output_channel(),
              ccomidi::kMaxSelectableOutputChannelIndex,
              "automated output channel clamps above channel 12");
    ASSERT_EQ(core.output_channel_value(), 11.0, "automated output channel stores the clamped value");
}
...
void test_start_of_playback_emits_snapshot()
{
    ...
    ASSERT_EQ(planned.count, 2, "playback start emits program change plus direct CC");
    ASSERT_EQ(planned.events[0].status, 0xC2, "snapshot leads with program change on configured channel");
    ASSERT_EQ(planned.events[1].time, 0, "playback snapshot is at time zero");
    ASSERT_EQ(planned.events[1].status, 0xB2, "configured channel overrides outgoing MIDI channel");
...
void test_fixed_rows_emit_expected_controllers()
{
    ...
    ASSERT_EQ(planned.count, 4, "snapshot emits program change then three fixed rows");
    ASSERT_EQ(planned.events[0].status & 0xF0, 0xC0, "snapshot leads with program change");
...
void test_channel_change_resends_snapshot()
{
    ...
    AutomationEvent event = make_event(12, ParamKind::OutputChannel, kVolumeRow, 5.9);
    core.process_block(TransportState{true}, &event, 1, &planned);

    ASSERT_EQ(planned.count, 2, "channel change resends program change and snapshot");
    ASSERT_EQ(planned.events[0].status, 0xC5, "resend leads with program change on new channel");
    ASSERT_EQ(planned.events[1].time, 12, "channel resend preserves event time");
    ASSERT_EQ(planned.events[1].status, 0xB5, "resend uses the new floored output channel");
    ASSERT_EQ(planned.events[1].data2, 64, "resend carries the selected channel bank value");
}
...
void test_compound_xcmd_row_reemits_full_sequence()
{
    ...
    core.set_row_type(kFirstConfigurableRow, static_cast<double>(CommandType::XcmdIecv));
    core.set_row_value(kFirstConfigurableRow, 0, 22.0);

    core.process_block(TransportState{true}, nullptr, 0, &planned);
    ASSERT_EQ(planned.count, 3, "xcmd snapshot emits program change, selector, and data");
    ASSERT_EQ(planned.events[0].status & 0xF0, 0xC0, "xcmd snapshot leads with program change");
    ASSERT_EQ(planned.events[1].data1, 0x1E, "xcmd selector controller emitted first");
    ASSERT_EQ(planned.events[1].data2, 0x08, "xcmd IECV selector value emitted");
    ASSERT_EQ(planned.events[2].data1, 0x1D, "xcmd data controller emitted second");
    ASSERT_EQ(planned.events[2].data2, 22, "xcmd data value emitted");

    AutomationEvent event = make_event(8, ParamKind::RowValue0, kFirstConfigurableRow, 23.9);
    core.process_block(TransportState{true}, &event, 1, &planned);
...
void test_stopped_automation_is_applied_on_next_start()
{
    ...
    AutomationEvent event = make_event(0, ParamKind::RowValue0, kVolumeRow, 43.8);
    core.process_block(TransportState{false}, &event, 1, &planned);
    ASSERT_EQ(planned.count, 0, "automation while stopped does not emit");

    core.process_block(TransportState{true}, nullptr, 0, &planned);
    ASSERT_EQ(planned.count, 2, "next playback start emits program change and cached value");
    ASSERT_EQ(planned.events[0].status & 0xF0, 0xC0, "snapshot leads with program change");
    ASSERT_EQ(planned.events[1].data2, 43, "playback start uses current automated state");
}
...
void test_memacc_reemits_full_canonical_sequence()
{
    ...
    core.set_row_enabled(kFirstConfigurableRow, 1.0);
    core.set_row_type(kFirstConfigurableRow, static_cast<double>(CommandType::MemAcc0C));
    ...
    core.process_block(TransportState{true}, nullptr, 0, &planned);
    ASSERT_EQ(planned.count, 5, "memacc snapshot emits program change and full setup sequence");
    ASSERT_EQ(planned.events[0].status & 0xF0, 0xC0, "memacc snapshot leads with program change");
    ...
    AutomationEvent event = make_event(9, ParamKind::RowValue3, kFirstConfigurableRow, 80.9);
    core.process_block(TransportState{true}, &event, 1, &planned);
...
void test_runtime_reset_preserves_parameter_values()
{
    ...
    ASSERT_EQ(planned.events[0].status & 0xF0, 0xC0, "snapshot leads with program change after reset");
    ASSERT_EQ(planned.events[1].data2, 100, "runtime reset preserves parameter values");
}
...
void test_preapplied_channel_change_resends_snapshot()
{
    ...
    core.apply_parameter_change(make_event(0, ParamKind::OutputChannel, kVolumeRow, 7.2), &channelChanged, &rowChanged);
    core.emit_preapplied_changes(true, channelChanged, rowChanged, 0, &planned);

    ASSERT_EQ(planned.count, 2, "preapplied channel change resends program change and snapshot");
    ASSERT_EQ(planned.events[0].status, 0xC7, "preapplied resend leads with program change on new channel");
    ASSERT_EQ(planned.events[1].status, 0xB7, "preapplied resend uses new output channel");
    ASSERT_EQ(planned.events[1].data2, 45, "preapplied resend carries selected channel bank value");
}
...
void test_fixed_rows_reject_type_changes()
{
    ...
    const bool changed = core.apply_parameter_change(
        make_event(0, ParamKind::RowType, kVolumeRow, static_cast<double>(CommandType::MemAcc0C)),
        &channelChanged,
        &rowChanged);
...
void test_channel_switch_keeps_single_command_bank()
{
    ...
    ASSERT_EQ(static_cast<int>(core.row_value_raw(kVolumeRow, 0)),
              100,
    ...
    ASSERT_EQ(planned.events[0].status, 0xC0, "snapshot leads with program change on channel 0");
    ASSERT_EQ(planned.events[1].status, 0xB0, "snapshot uses current output channel");
    ASSERT_EQ(planned.events[1].data2, 100, "snapshot keeps the configured single-bank value");

    AutomationEvent switchEvent = make_event(5, ParamKind::OutputChannel, kVolumeRow, 1.0);
    ...
    ASSERT_EQ(planned.events[0].status, 0xC1, "switch resend leads with program change on new channel");
    ASSERT_EQ(planned.events[1].status, 0xB1, "switch resend uses the new output channel");
    ASSERT_EQ(planned.events[1].data2, 100, "switch resend preserves the configured value");
}
...
void test_xcmd_atta_emits_selector_and_data()
{
    ...
    core.set_output_channel(0.0);
    core.set_row_enabled(kFirstConfigurableRow, 1.0);
    core.set_row_type(kFirstConfigurableRow, static_cast<double>(CommandType::XcmdAtta));
    core.set_row_value(kFirstConfigurableRow, 0, 90.0);

    core.process_block(TransportState{true}, nullptr, 0, &planned);
    ASSERT_EQ(planned.count, 3, "xATTA snapshot emits program change, selector, and data");
...
void test_full_snapshot_with_4byte_xcmds_does_not_truncate()
{
    ...
    for (std::size_t row = kFirstConfigurableRow; row < ccomidi::kMaxCommandRows; ++row)
    {
        core.set_row_enabled(row, 1.0);
    ...
    ASSERT_EQ(planned.count, 65, "full snapshot with 4-byte xCmds emits every byte");
    ASSERT_EQ(planned.events[0].status & 0xF0, 0xC0, "snapshot leads with program change");
    // Last byte must be the final Xcmd0D payload byte (0x44), not truncated.
    ASSERT_EQ(planned.events[planned.count - 1].data1, 0x1D, "last event is a payload byte");
    ASSERT_EQ(planned.events[planned.count - 1].data2, 0x44, "last payload byte survives without truncation");
}
...
void test_xcmd_0d_emits_four_data_bytes()
{
    ...
    core.set_output_channel(0.0);
    core.set_row_enabled(kFirstConfigurableRow, 1.0);
    core.set_row_type(kFirstConfigurableRow, static_cast<double>(CommandType::Xcmd0D));
    ...
    core.process_block(TransportState{true}, nullptr, 0, &planned);
    ASSERT_EQ(planned.count, 6, "XCMD 0D snapshot emits program change, selector, and 4 data bytes");
