...
{
    return msg(static_cast<std::uint8_t>(0x80 | channel), static_cast<std::uint8_t>(note), 0);
}
...
void advance_ticks(LegatoBendCore* core, int count, std::vector<MidiEvent>* out)
{
    for (int i = 0; i < count; ++i)
        core->advance(5.0, out);
}

void assert_events(const std::vector<MidiEvent>& actual, const std::vector<MidiEvent>& expected, const char* label)
{
...
void test_anchor_release_during_glide_defers_note_off_until_target_release()
{
    ...
    feed(&core, note_off(64), &out);
    assert_events(out,
                  {cc(0x14, 32), note_on(60), msg(0xE0, 0, 65), msg(0xE0, 0, 66), note_off(60)},
...
void test_bend_range_cc_scales_target_to_mp2k_semitones()
{
    ...
    feed(&core, note_off(60), &out);
    assert_events(out,
                  {cc(0x14, 32), note_on(60), msg(0xE0, 0, 65), msg(0xE0, 0, 66), note_off(60), msg(0xE0, 0, 64)},
...
void test_zero_bend_time_passes_notes_through()
{
    ...
    feed(&core, note_off(64), &out);
    feed(&core, note_off(60), &out);
    assert_events(out, {note_on(60), note_on(64), note_off(64), note_off(60)}, "disabled core passes through");
}
