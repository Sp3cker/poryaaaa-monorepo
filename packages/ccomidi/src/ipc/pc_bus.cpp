...
bool PCBus::open()
{
    ...
    const std::uint32_t observedMagic = shared_->magic.load(std::memory_order_acquire);
...
void PCBus::publish(std::uint8_t channel, const PCBusSlot& slot)
{
    ...
    const std::uint64_t pre = entry.seq.load(std::memory_order_relaxed);
...
bool PCBus::read_slot(std::uint8_t channel, PCBusSlot* out, std::uint64_t* seq_out) const
{
    ...
    {
        const std::uint64_t pre = entry.seq.load(std::memory_order_acquire);
        if ((pre & 1u) != 0u)
        {
        ...
        const std::uint64_t post = entry.seq.load(std::memory_order_acquire);
        if (pre == post)
        {
            *out = copy;
            if (seq_out)
