...
private:
    ...
    void
    process_note_on(Channel* channel, std::uint8_t midiChannel, const MidiEvent& event, std::vector<MidiEvent>* out);
    void
    process_note_off(Channel* channel, std::uint8_t midiChannel, const MidiEvent& event, std::vector<MidiEvent>* out);
    ...
    void finish_phrase_with_generated_note_off(Channel* channel, std::uint8_t midiChannel, std::vector<MidiEvent>* out);
    void append_reset_if_needed(Channel* channel, std::uint8_t midiChannel, std::vector<MidiEvent>* out);
    static void reset_phrase(Channel* channel);
    static void append_pitch_bend(std::uint8_t midiChannel, int value, std::vector<MidiEvent>* out);
