...
void LegatoBendCore::process(const MidiEvent& event, std::vector<MidiEvent>* out)
{
    if (!out)
        return;
    if (!enabled())
...
void LegatoBendCore::advance(double elapsedMs, std::vector<MidiEvent>* out)
{
    ...
    if (!out || elapsedMs <= 0.0)
        return;
    for (auto midiChannel = std::uint8_t{0}; midiChannel < channels_.size(); ++midiChannel)
    {
        auto& channel = channels_[midiChannel];
        ...
        double progress = ramp.durationMs > 0.0 ? ramp.elapsedMs / ramp.durationMs : 1.0;
...
double LegatoBendCore::bend_time_ms() const
{
    return bendTimeMs_;
}

BendCurve LegatoBendCore::bend_curve() const
{
    return bendCurve_;
}

bool LegatoBendCore::enabled() const
{
    return bendTimeMs_ > 0.0;
}

bool LegatoBendCore::has_active_ramp() const
{
    return std::any_of(channels_.begin(), channels_.end(), [](const Channel& channel) { return channel.ramp.active; });
}
...
void LegatoBendCore::finish_phrase_with_generated_note_off(Channel* channel,
                                                           ...
                                                           std::vector<MidiEvent>* out)
{
    out->push_back(MidiEvent{static_cast<std::uint8_t>(kNoteOff | midiChannel), channel->anchorNote, 0});
    reset_phrase(channel);
}

void LegatoBendCore::append_reset_if_needed(Channel* channel, std::uint8_t midiChannel, std::vector<MidiEvent>* out)
{
...
void LegatoBendCore::append_pitch_bend(std::uint8_t midiChannel, int value, std::vector<MidiEvent>* out)
{
    out->push_back(MidiEvent{
        static_cast<std::uint8_t>(kPitchBend | midiChannel), 0, static_cast<std::uint8_t>(clamp_data_byte(value))});
}
...
int LegatoBendCore::target_bend_value(const Channel& channel, int targetNote)
{
    ...
    const auto bendUnits = static_cast<int>(std::lround(double(semitones * 64) / double(channel.bendRange)));
