...
constexpr std::uint8_t kMaxSelectableOutputChannelIndex = static_cast<std::uint8_t>(kSelectableOutputChannelCount - 1);
...
{
    return type == CommandType::Volume || type == CommandType::Pan || type == CommandType::Mod ||
           type == CommandType::LfoSpeed;
}
...
private:
    ...
    bool apply_event(const AutomationEvent& event,
                     bool* channelChanged,
                     std::array<bool, kMaxCommandRows>* rowChanged,
                     bool* programChanged);
