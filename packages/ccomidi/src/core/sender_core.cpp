...
{
    outputChannelValue_ = static_cast<double>(floor_to_u8(value, 0, kMaxSelectableOutputChannelIndex));
}

void SenderCore::set_program(double value)
{
    programValue_ = value;
}
...
void SenderCore::set_row_type(std::size_t row, double value)
{
    if (row >= kMaxCommandRows)
        return;
    rows_[row].typeValue = static_cast<double>(sanitize_row_type(row, floor_to_command_type(value)));
}
...
double SenderCore::program_value() const
{
    return programValue_;
}
...
{
    const auto raw = floor_to_u8(
        value, static_cast<std::uint8_t>(CommandType::None), static_cast<std::uint8_t>(CommandType::Xcmd0D));
                             ...
                             bool* programChanged)
{
    ...
    case ParamKind::RowType:
            ...
            return false;
        {
            ...
            rows_[event.address.row].typeValue =
                static_cast<double>(sanitize_row_type(event.address.row, floor_to_command_type(event.value)));
    ...
    case ParamKind::RowValue3:
            ...
            return false;
        {
            const std::size_t field =
                static_cast<std::size_t>(event.address.kind) - static_cast<std::size_t>(ParamKind::RowValue0);
                               ...
                               PlannedEvents* out)
{
    ...
    while (eventIndex < eventCount || (!startEdgeHandled && transport.isPlaying && !lastTransportPlaying_))
    {
        const std::uint32_t currentTime =
            ...
            : (!startEdgeHandled && transport.isPlaying && !lastTransportPlaying_) ? 0
        ...
        while (eventIndex < eventCount && events[eventIndex].time == currentTime)
        {
            ...
            apply_event(events[eventIndex], &localChannelChanged, &rowChanged, &localProgramChanged);
