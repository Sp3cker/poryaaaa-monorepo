...
void draw_frame(void* userData, std::uint32_t width, std::uint32_t height)
{
    ...
    float bendTime = static_cast<float>(std::clamp(snapshot.bendTimeMs, 0.0, static_cast<double>(kMaxBendTimeMs)));
    ImGui::SetNextItemWidth(-1.0f);
    if (ImGui::SliderFloat(
            "Bend Time", &bendTime, 0.0f, static_cast<float>(kMaxBendTimeMs), "%.1f ms", ImGuiSliderFlags_AlwaysClamp))
    {
