...
EditorShell*
editor_shell_create(const clap_host_t* host, const EditorShellConfig& config, const EditorShellCallbacks& callbacks)
{
    ...
    editorShell->shell = poryaaaa::gui::imgui_pugl_shell_create(shellConfig, shellCallbacks);
...
{
    return shell && poryaaaa::gui::imgui_pugl_shell_set_parent(shell->shell, nativeParent);
}
