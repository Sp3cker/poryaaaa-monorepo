...
//  [X] Renderer: User texture binding. Use 'MTLTexture' as texture identifier.
...
//  dynamic font atlas. Removed ImGui_ImplMetal_CreateFontsTexture() and
//  ImGui_ImplMetal_DestroyFontsTexture(). 2025-02-03: Metal: Crash fix. (#8367)
//  2025-01-08: Metal: Fixed memory leaks when using metal-cpp (#8276, #8166) or
...
void ImGui_ImplMetal_NewFrame(MTLRenderPassDescriptor *renderPassDescriptor) {
  ImGui_ImplMetal_Data *bd = ImGui_ImplMetal_GetBackendData();
  IM_ASSERT(bd != nil && "Context or backend not initialized! Did you call "
...
void ImGui_ImplMetal_UpdateTexture(ImTextureData *tex) {
  ...
  if (tex->Status == ImTextureStatus_WantCreate) {
    ...
    // We are retrieving and uploading the font atlas as a 4-channels RGBA
    // texture here. In theory we could call GetTexDataAsAlpha8() and upload a
    // 1-channel texture to save on memory access bandwidth. However, using a
    // shader designed for 1-channel texture would make it less obvious to use
    // the ImTextureID facility to render users own textures. You can make that
