...
> Calculate the maximum of the numerical elements of an array
...
- `anything(ARG_NAME_0: list)` — Convert a list to an array.
  Convert an incoming list to an array, then process as described for the array message.
- `array(ARG_NAME_0: list)` — Calculate the maximum
...
`array`, `array.min`, `array.mean`, `array.median`, `array.mode`, `array.stddev`, `maximum`
