...
static std::uint8_t max_channel_to_nibble(long channel)
{
    if (channel >= 1 && channel <= 16)
        return (std::uint8_t)((channel - 1) & 0x0F);
    if (channel >= 0 && channel <= 15)
        return (std::uint8_t)(channel & 0x0F);
    return 0;
}
...
static void emit_status_data(t_cco* x, std::uint8_t status, std::uint8_t d1, std::uint8_t d2)
{
    ...
    if (n == 2)
        outlet_int(x->out_midi, d2);
}

static void emit_channel_event(t_cco* x, std::uint8_t high_nibble, std::uint8_t d1, std::uint8_t d2, long channel = 0)
{
...
{
    if (!include_neutral && !optional_active(x, opt))
        return;

    switch (opt)
    {
    case OptMod:
        emit_cc(x, kCcMod, x->modVal);
        break;
    case OptLfoSpeed:
        emit_cc(x, kCcLfoSpeed, x->lfoSpdVal);
        break;
    case OptLfoDelay:
        emit_cc(x, kCcLfoDelay, x->lfoDlyVal);
        break;
    case OptBendRange:
        emit_cc(x, kCcBendRange, x->bendRangeVal);
        break;
    case OptModType:
        emit_cc(x, kCcModType, x->modTypeVal);
        break;
    case OptTune:
        emit_cc(x, kCcTune, x->tuneVal);
        break;
    case OptIecv:
        emit_xcmd(x, kXcmdIecv, x->iecvVal);
        break;
    case OptIecl:
        emit_xcmd(x, kXcmdIecl, x->ieclVal);
        break;
    case OptPriority:
        emit_priority(x, include_neutral);
        break;
    default:
        break;
    }
...
static void cco_sendall(t_cco* x)
{
    emit_full_snapshot(x);
}
...
{
    for (long& pending : x->pendingClear)
        pending = 0;
}
...
{
    t_class* c = class_new("ccomidi", (method)cco_new, (method)cco_free, (long)sizeof(t_cco), 0L, A_GIMME, 0);
