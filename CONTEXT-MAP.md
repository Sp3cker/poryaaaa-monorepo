...
- [poryaaaa Max for Live](./packages/poryaaaa-m4l/CONTEXT.md) — Ableton-set-scoped device state, patcher routing, Node-for-Max sidecars, and WebSocket voicegroup transport for poryaaaa/ccomidi devices.
...
- **poryaaaa Max for Live → Voicegroup Core**: Node-for-Max asks generated voicegroup-core bindings to scan and parse the Live-set voicegroup selection, then broadcasts slot state to ccomidi over WebSocket.
